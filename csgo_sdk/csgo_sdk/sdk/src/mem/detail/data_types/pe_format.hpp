...
#pragma pack( push, 4u )
	...
	struct dos_hdr_t {
		std::uint16_t   m_magic{}, m_cыp{},
						m_cp{}, m_crlc{}, m_cparhdr{},
						m_min_alloc{}, m_max_alloc{},
						m_ss{}, m_sp{}, m_csum{},
						m_ip{}, m_cs{}, m_lfarlc{},
						m_ovno{}, m_res[ 4u ]{}, m_oem_id{},
						m_oem_info{}, m_res2[ 10u ]{};

		std::uint32_t	m_lfanew{};
	};
	...
	struct pe_image_t {
		...
		ALWAYS_INLINE nt_hdrs_t< _x64 >* nt_hdrs( ) {
			return reinterpret_cast< nt_hdrs_t< _x64 >* >(
				reinterpret_cast< std::uint8_t* >( this ) + m_dos_hdr.m_lfanew
			);
		}
		...
		ALWAYS_INLINE bool for_each_export( const addr_t base_addr,
			...
			const auto export_dir = base_addr.offset( data_dir.m_rva ).as< export_dir_t* >( );

			const auto names = base_addr.offset( export_dir->m_names_rva ).as< std::uint32_t* >( );
			const auto fns = base_addr.offset( export_dir->m_fns_rva ).as< std::uint32_t* >( );
			const auto name_ords = base_addr.offset( export_dir->m_name_ords_rva ).as< std::uint16_t* >( );

			for ( std::uint32_t i{}; i < export_dir->m_names_count; ++i )
				if ( fn( base_addr.offset( names[ i ] ).as< const char* >( ), base_addr.offset( fns[ name_ords[ i ] ] ) ) )
					break;
