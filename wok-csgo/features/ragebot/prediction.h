...
private:
	...
	int post_think(c_cs_player* player) {
		...
		if (player->is_alive()
			...
			static const auto post_think_v_physics_fn = reinterpret_cast<bool(__thiscall*)(void*)>(SIG("client.dll", "55 8B EC 83 E4 F8 81 EC ? ? ? ? 53 8B D9 56 57 83 BB"));
			post_think_v_physics_fn(player);
		}

		static const auto simulate_player_simulated_entities_fn = reinterpret_cast<bool(__thiscall*)(void*)>(SIG("client.dll", "56 8B F1 57 8B BE ? ? ? ? 83 EF 01 78 72"));
...
public:
	c_prediction() {
		...
		m_prediction_player = *reinterpret_cast<int**>(SIG("client.dll", "89 35 ? ? ? ? F3 0F 10 48") + 0x2);
		m_prediction_random_seed = *reinterpret_cast<int**>(SIG("client.dll", "A3 ? ? ? ? 66 0F 6E 86") + 0x1);
	}
